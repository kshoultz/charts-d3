...
d3.charts.MapAzimuth = function (config) {
    ...
    var projectionMenu;
    //d3.charts.projections.currentProjectionIndex = config.projectionIndex || 17;
    ...
    // Projection Menu: 
    projectionMenu = d3.charts.projections.getProjectionMenu(width, height, boundingClientRect.left, boundingClientRect.top);
    projectionMenu.on("change", function () {
        d3.charts.projections.currentProjectionIndex = this.selectedIndex;
        update(d3.charts.projections.options[this.selectedIndex]);
    });

    // Projection Definitions:
    projection = d3.charts.projections.options[0].projection;
    ...
    spaceProjection = d3.geo.azimuthal()
        ...
        .origin([-71.03, 42.37])
        .mode("gnomonic")
        .translate([((width / 2)-boundingClientRect.left), ((height / 2)-boundingClientRect.top)]);
    ...
    function update(option) {
        if (option.name !== "Azimuthal" && option.projection.name !== "azimuthal") {
            ...
            gOceans.interrupt().transition()
                ...
                .attrTween("d", projectionTween(projection, projection1 = d3.charts.projections.translate(option.projection)));

            gGraticules.interrupt().transition()
                .duration(1000).ease(d3.easeLinear)
                .attrTween("d", projectionTween(projection, projection2 = d3.charts.projections.translate(option.projection)));

            gLands.interrupt().transition()
                .duration(1000).ease(d3.easeLinear)
                .attrTween("d", projectionTween(projection, projection3 = d3.charts.projections.translate(option.projection)));

            gLocations.interrupt().transition()
                .duration(1000).ease(d3.easeLinear)
                .attrTween("d", projectionTween(projection, projection4 = d3.charts.projections.translate(option.projection)));

            gRoutes.interrupt().transition()
                .duration(1000).ease(d3.easeLinear)
                .attrTween("d", projectionTween(projection, projection5 = d3.charts.projections.translate(option.projection)));
            
        }
            projection = d3.charts.projections.translate(option.projection);
            path = d3.geo.path().projection(projection);
    ...
    me.switchProjection = function (name) {
        var projection = d3.charts.projections.options[0];
        d3.charts.projections.options.forEach(function (option, i) {
            if (option.name === name) {
                projection = option;
...
d3.charts.projections = {
    currentProjectionIndex: 0,// 17,
    width:undefined,
    height:undefined,
    xOffset:0,
    yOffset:0,
    scale:300,
    translate:function(projection){ console.log(this.xOffset,this.yOffset); return projection.translate([((this.width / 2) - this.xOffset),((this.height / 2) - this.yOffset)]); },
    getProjection: function(index) {
        return this.translate(this.options[index].projection);
    },
    getProjectionMenu: function (width, height, xOffset, yOffset) {
        var me = this;
        this.width = width;
        this.height = height;
        ...
        this.options.forEach(function (o) {
            me.translate(o.projection);
        });
        this.projectionMenuOptions = this.projectionMenu.selectAll("option")
            .data(this.options)
            .enter().append("option")
            .attr("selected", function (d, i) {
                if (this.currentProjectionIndex === i) { return "selected"; } else { return null; }
            })
            .text(function (d) { return d.name; });
        return this.projectionMenu;
    },
    projectionMenu:d3.select("#projection-menu"),
    projectionMenuOptions:undefined,
    options: options = [
